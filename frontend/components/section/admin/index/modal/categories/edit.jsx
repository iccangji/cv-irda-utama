...
const EditCategoryModal = ({ product, onClose, onSuccess }) => {
    ...
    const handleEditData = async (e) => {
        ...
        if (
            ...
            setLoading(false);
            setError("Pastikan terdapat data yang diubah");
            return;
        }

        try {
            const token = Cookies.get('token');
            const response = await axios.put(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/categories/${product.id}`,
                {
